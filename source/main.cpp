...
///////////////////////////#Подготовка#////////////////////////////////
  ...
  double* extra_mem = new double[n*n + n];
  ...
  for (int i = 0; i < total_threads; i++)
    printf("Поток %d: Процессорное: %.3lf    Астрономическое: %.3lf\n",
     args[i].id, args[i].time_cpu, args[i].time_ast);

  printf("Норма невязки: %.4lf\n", residual);
  printf("Норма погрешности: %.4lf\n", Inaccuracy(x, n));
